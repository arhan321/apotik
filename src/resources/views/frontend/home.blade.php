    ...
    <div class="col-lg-3 mb-4">
      ...
      <div class="list-group kategori-list shadow-sm">
        ...
        @foreach ($kategoriList as $kategori)
          <a href="#" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" data-kategori="{{ $kategori }}">
            {{ $kategori }} <i class="bi bi-plus-circle-fill"></i>
          </a>
        @endforeach
    ...
    <div class="col-lg-9">
      <div class="row g-4">
        @foreach ($obats as $obat)
          <div class="col-6 col-md-4 col-lg-3" data-kategori="{{ $obat->jenis->name }}">
            <div class="card card-obat h-100 position-relative" data-obatid="{{ $obat->id }}">
              <span class="badge {{ $obat->status_label == 'Tanpa Resep' ? 'bg-success' : 'bg-danger' }} text-white position-absolute top-0 start-0 rounded-end px-2 py-1 small z-1">
                {{ $obat->status_label }}
              </span>
              <div class="img-hover-container">
                <img src="{{ asset('storage/' . $obat->image) }}" class="card-img-top" alt="{{ $obat->nama_obat }}">
              </div>
              <div class="card-body px-3 pt-3">
                <p class="text-muted small mb-1 d-flex justify-content-start align-items-center gap-1">
                  {{ $obat->jenis->name ?? '-' }}
                </p>
                <h6 class="fw-semibold mb-2 small">{{ $obat->nama_obat }}</h6>
              </div>
              <div class="card-footer d-flex justify-content-between align-items-center px-3 py-2 bg-white border-0">
                <span class="harga-produk fw-bold">Rp{{ number_format($obat->harga, 0, ',', '.') }}</span>
                @if ($obat->status_label == 'Tanpa_Resep')
